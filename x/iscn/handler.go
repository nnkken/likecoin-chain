...
func handleStakeholders(ctx sdk.Context, stakeholders IscnData, keeper Keeper) (*CID, error) {
	...
	for i := 0; i < stakeholdersArr.Len(); i++ {
		...
		stakeholder.Set("entity", *cid)
		n, _ := stakeholder.Get("sharing").AsUint64()
		stakeholder.Set("sharing", uint32(n))
	}
	return keeper.SetCidIscnData(ctx, stakeholders, StakeholdersCodecType)
}
...
func handleRights(ctx sdk.Context, rights IscnData, keeper Keeper) (*CID, error) {
	...
	return keeper.SetCidIscnData(ctx, rights, RightsCodecType)
}
...
func handleMsgCreateIscn(ctx sdk.Context, msg MsgCreateIscn, keeper Keeper) sdk.Result {
	...
	case NestedCID:
		...
		parentKernel := keeper.GetIscnKernelByCID(ctx, *parentKernelCID)
		if parentKernel == nil {
			return sdk.Result{
				Code:      123, // TODO
				Codespace: DefaultCodespace,
		...
		parentVersion, _ := parentKernel.Get("version").AsUint64()
		...
		iscnID, _ := parentKernel.Get("id").AsBytes()
